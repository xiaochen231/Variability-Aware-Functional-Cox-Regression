...
grid_y <- c(rep("Censoring = 10%", 600*length(n1_plot_all)*length(rho_plot_all)*length(q_plot_all)*length(q_true_plot_all)),  
...
grid_y <- factor(grid_y, levels = c(  "Censoring = 10%", "Censoring = 20%",  "Censoring = 30%",  "Censoring = 50%"))
...
p_TP <- ggplot(data = data_TP, aes(x = method, y = MAE))  + 
  ...
  theme(axis.title.y = element_text(size = 22), axis.text.x = element_text(size = 10), 
